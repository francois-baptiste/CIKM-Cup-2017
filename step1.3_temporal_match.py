...
for set_name, N_slice in zip(set_name_list, N_slice_list):
    ...
    for slice_id1 in range(1, N_slice + 1):
        ...
        search_list = list(range(max(1, slice_id1 - 2), slice_id1)) + list(
            range(slice_id1 + 1, min((slice_id1 + 4), N_slice + 1)))
                                ...
                                for time_diff in time_diff_list:
                                    ...
                                    if ((T_id2 > 0) & (T_id2 < 16)):
                                        ...
                                        if len(match_result[0]) == 1:
                                            ...
                                            match_all.append((slice_id1, row_cent1, col_cent1, T_id1, slice_id2,
                                                              row_cent2, col_cent2, T_id2))

    match_all_pd = pd.DataFrame(match_all,
                                columns=['slice_id1', 'row_id1', 'col_id1', 'T_id1', 'slice_id2', 'row_id2', 'col_id2',
                                         'T_id2'])
    ...
    for ind, value in match_all_pd.iterrows():
        if (value.slice_id2 < value.slice_id1):
            ...
            match_all_pd.loc[ind, ['T_id1', 'col_id1', 'row_id1', 'slice_id1']] = match_all_pd.loc[
                ind, ['T_id2', 'col_id2', 'row_id2', 'slice_id2']].values
            match_all_pd.loc[ind, ['T_id2', 'col_id2', 'row_id2', 'slice_id2']] = temp
            match_all_pd.loc[ind, ['row_diff', 'col_diff', 'time_diff']] = - match_all_pd.loc[
                ind, ['row_diff', 'col_diff', 'time_diff']]
    ...
    Pano_set_org = Set([1])
    for ind, value in match_all_pd.iterrows():
        Pano_set_new = Set(list(range(value.slice_id1, value.slice_id2 + 1)))
    ...
    for ind, value in Pano_time_pd.iterrows():
        ...
        slice_range = list(range(slice_sta, slice_end + 1))
        ...
        for ind2 in range(slice_len):
            ...
            for edge_ind in range(len(edge_path) - 1):
                ...
                path_line = slices[
                    (slices.slice_id1 == small_one + slice_sta) & (slices.slice_id2 == big_one + slice_sta)]
