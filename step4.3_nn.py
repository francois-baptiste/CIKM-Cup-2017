...
with tf.Session() as sess:
    ...
    split_ind = 4342
    for i in range(2001):
        #        _,loss_value= sess.run([train_step,cost],feed_dict={x:trn_patch[split_ind::,:],y_true:Y[split_ind::],keep_prob:0.6})
        #        _,loss_value= sess.run([train_step,cost],feed_dict={x:trn_patch[0:split_ind:,:],y_true:Y[0:split_ind],keep_prob:0.7})
...
tst_pic.to_csv(data_folder + 'result_nn.csv', index=False)

valid_curve = pd.DataFrame({'iter': iter_list, 'train_loss': train_loss_list, 'val_loss': val_loss_list})
valid_curve.to_csv(data_folder + 'result_nn_curve.csv', index=False)
