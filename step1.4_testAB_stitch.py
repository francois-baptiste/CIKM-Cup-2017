...
import numpy as np
import pandas as pd
from cikm_tools import cv2_based, read_slice
        ...
        for H_id in range(1, 5):
            ...
            for row_cent1 in range(N_pad, N_row - 1 - N_pad, 30):
                for col_cent1 in range(N_pad, N_col - 1 - N_pad, 30):
                    ...
                    img_temp = img1[(row_cent1 - N_pad): (row_cent1 + N_pad + 1),
                    ...
                    if (np.sum([img_temp == 200]) == 0) & (np.sum([img_temp > 1]) >= 0.2 * N_pixel):
                        for slice_id2 in search_list:
                            ...
                            if len(match_result[0]) == 1:
                                ...
                                match_all.append(
                                    (slice_id1, row_cent1, col_cent1, T_id1, slice_id2, row_cent2, col_cent2, T_id2))
                                N_match = N_match + 1

match_all_pd = pd.DataFrame(match_all)
match_all_pd.columns = ['testA_SLI_ID', 'testA_ROW_ID', 'testA_COL_ID', 'testA_TIM_ID', 'testB_SLI_ID', 'testB_ROW_ID',
                        'testB_COL_ID', 'testB_TIM_ID']
...
for sam_id in range(1, 1 + testB_MATCH.SAM_ID.max()):
    ...
    if len(testAB_match) > 0:
        ...
        testAB_match = pd.merge(testAB_match, SAMSA[['testA_SLI_ID', 'testA_row_sta', 'testA_col_sta', 'testA_TIM_ID']],
                                on='testA_SLI_ID', how='left')
        testAB_match = pd.merge(testAB_match, SAMSB[['testB_SLI_ID', 'testB_row_sta', 'testB_col_sta', 'testB_TIM_ID']],
                                on='testB_SLI_ID', how='left')
            ...
            for H_id in range(1, 5):
                ...
                for ind, value in SAMSB.iterrows():
                    ...
                    if ((T_id >= 1) & (T_id <= 15)):
                        ...
                        view_all[ROW_SHIFT_B + useful_place[0] + row_id, COL_SHIFT_B + useful_place[1] + col_id] = \
                            data_mat[useful_place]
                if TH_ind == 0:
                    sample_stat.append((SAMSA.SAM_ID.values[0], SAMSB.SAM_ID.values[0], ROW_SIZE, COL_SIZE, TIME_MAX,
                                        size_all, ROW_SHIFT_A, COL_SHIFT_A, ROW_SHIFT_B, COL_SHIFT_B))
...
sample_stat_pd = pd.DataFrame(sample_stat,
                              columns=['testA_SAM_ID', 'testB_SAM_ID', 'N_row', 'N_col', 'N_time', 'start_pos',
                                       'testA_ROW_SHIFT', 'testA_COL_SHIFT', 'testB_ROW_SHIFT', 'testB_COL_SHIFT'])
...
slice_sample = pd.merge(slice_sample, sample_size[['testB_SAM_ID', 'testB_ROW_SHIFT', 'testB_COL_SHIFT']], how='left',
                        left_on='SAM_ID', right_on='testB_SAM_ID')
