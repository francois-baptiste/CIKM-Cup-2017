...
with tf.Session() as sess:
    ...
    for i in range(1201):
        ...
        _, loss_value = sess.run([train_step, cost],
                                 feed_dict={x: trn_img, y_true: Y, features: trn_flat, keep_prob: 0.65})
        if i % 50 == 0:
            train_loss = (cost.eval(
                feed_dict={x: trn_img[0:split_ind, :, :, :], y_true: Y[0:split_ind], features: trn_flat[0:split_ind],
                           keep_prob: 1.0}) / len(Y[0:split_ind])) ** 0.5
            val_loss = (cost.eval(
                feed_dict={x: trn_img[split_ind::, :, :, :], y_true: Y[split_ind::], features: trn_flat[split_ind::],
                           keep_prob: 1.0}) / len(Y[split_ind::])) ** 0.5
            iter_list.append(i)
